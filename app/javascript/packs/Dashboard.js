...
export default function Dashboard() {
  ...
  const getUserUploads = () => {
    axios
      .get(`/api/user_uploads/${localStorage.user_id}`)
      .then((res) => {
  ...
  const uploadToAwsS3 = async () => {
    if (file && fileInfo.name && fileInfo.description) {
      ...
      const { data } = await axios.post(
        "/api/user_uploads/generate_presigned_url",
        {
          fileName: file.name,
          fileType: file.type,
  ...
  const handleDeleteFile = (obj) => {
    axios
      .delete(`/api/user_uploads/${obj.id}`, {
        data: {
          key: obj.key,
        },
      })
  ...
  const handleShareDoneBtn = () => {
    axios
      .put(`/api/short_links/${userUploadItem.id}`, {
        access: sharedAccess,
        emails: sharedUsers,
      })
      .then((res) => {
          ...
          <DialogContent>
            <div className="file-input">
              <input
                ...
                style={{ display: "none" }}
              />
              <span onClick={uploadFile} className="button">
                Choose
              </span>
            ...
            <div className="share-dialog">
              {!generatedLink ? (
                <>
                  ...
                  >
                    There is no short url generated, click on Generate link
                  </Typography>
                ...
                <>
                  <div className="generated-link">
                    <span className="link-text">{generatedLink.short_url}</span>
                    <Button onClick={handleLinkCopyBtn}>
                      {linkCopied.copied ? "Copied" : "Copy"}
                    </Button>
                  </div>
                  <Typography
                    style={{ marginTop: "20px", marginBottom: "20px" }}
                  ...
                  <Stack direction="row">
                    <Checkbox
                      checked={sharedAccess === "public"}
                      onChange={() => setSharedAccess("public")}
                    >
                      Public
                    </Checkbox>
                    <Checkbox
                      checked={sharedAccess === "private"}
                      onChange={() => setSharedAccess("private")}
                    >
                      Private
                    </Checkbox>
